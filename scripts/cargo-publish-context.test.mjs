...
const context = { packageName: 'stack-formatter', version: '0.1.0', expectedSha: sha, publish: 'false' };
...
test('accepts exact successful source for verification or publication', () => {
  ...
  for (const name of ["stack-formatter","stack-engine"]) validatePublish({ packages: [{ ...crate, name }] }, runs, { ...context, packageName: name });
});
