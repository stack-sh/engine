...
const packages = ["stack-formatter","stack-engine"];
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  const status = async url => (await fetch(url, { headers: { 'User-Agent': 'stack-sh/engine publication (https://github.com/stack-sh/engine)' }, signal: AbortSignal.timeout(30000) })).status;
